...
impl fmt::Display for DriverError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            DriverError::ComposeFailed { command, stderr } => write!(f, "`{command}` failed: {stderr}"),
...
impl ServiceDriver {
    ...
    pub async fn ensure_started(&self) -> Result<(), DriverError> {
        match self {
            ServiceDriver::Container(d) => d.ensure_started().await,
            ServiceDriver::Compose(d) => d.ensure_started().await,
        }
    }

    /// Cooldown-triggered scale-down. Always a `stop`, never a `down` — a
    /// compose-driven stack keeps its containers/networks around so the next
    /// activation is a fast `start`, not a full `up -d` reconciliation.
...
pub fn build_all(
    ...
    for (name, svc) in &catalog.services {
        let driver = build_one(name, svc, config_dir, docker.clone(), &network);
...
fn build_one(
    ...
    if let Some(container) = &svc.container {
        return ServiceDriver::Container(build_container_driver(name, svc, container, docker, network));
    }
